 ...
 */
//javaapk.com 安卓源码服务专家提供下载
package com.net.margaritov.preference.colorpicker;
...
public class ColorPickerView extends View {
	...
	private void init(){
		mDensity = getContext().getResources().getDisplayMetrics().density;//閼惧嘲褰囩仦蹇撶鐎靛棗瀹抽獮璺哄灥婵瀵叉稉澶婂隘閸╃喎鎮囨い鐟板棘閺侊拷		PALETTE_CIRCLE_TRACKER_RADIUS *= mDensity;
		RECTANGLE_TRACKER_OFFSET *= mDensity;
		HUE_PANEL_WIDTH *= mDensity;
		ALPHA_PANEL_HEIGHT *= mDensity;
		PANEL_SPACING = PANEL_SPACING * mDensity;

		mDrawingOffset = calculateRequiredOffset();//鐠侊紕鐣婚幍锟芥付娴ｅ秶些

		initPaintTools();//閸掓繂顫愰崠鏍帛閸掓湹绗侀崠鍝勭厵閻ㄥ嫮鏁剧粭锟�		
		//Needed for receiving trackball motion events. 鐠佸墽鐤嗛悞锔惧仯
		setFocusable(true);
	...
	protected void onDraw(Canvas canvas) {
		...
		drawSatValPanel(canvas);//缂佹ê鍩楁鍗炴嫲鎼达箓锟介幏鈺佸隘閸╋拷		
		drawHuePanel(canvas);//缂佹ê鍩楅崣鍏呮櫠閼硅尙娴夐柅澶嬪閸栧搫鐓�		
		drawAlphaPanel(canvas);//缂佹ê鍩楁惔鏇㈠劥闁繑妲戞惔锕傦拷閹封晛灏崺锟�	
		}

	/**
	 * 缂佹ê鍩楁鍗炴嫲鎼达箓锟介幏鈺佸隘閸╋拷	 
	 * * @param canvas
	 */
	private void drawSatValPanel(Canvas canvas){
		...
		//缂佸嫬鎮庡〒鍙夌厠 = 閺勫骸瀹崇痪鎸庯拷濞撳弶鐓嬮崳锟� 妤楀崬鎷扮痪鎸庯拷濞撳弶鐓嬮崳锟�	
		ComposeShader mShader = new ComposeShader(mValShader, mSatShader, PorterDuff.Mode.MULTIPLY);
		mSatValPaint.setShader(mShader);

		canvas.drawRect(rect, mSatValPaint);
		//閸掓繂顫愰崠鏍拷閹封晛娓鹃崸妤冩畱娴ｅ秶鐤�		
	...
	/**
	 * 缂佹ê鍩楁惔鏇㈠劥闁繑妲戞惔锕傦拷閹封晛灏崺锟� * @param canvas
	 */
	private void drawAlphaPanel(Canvas canvas){
		...
		float[] hsv = new float[]{mHue,mSat,mVal};//hsv閺佹壆绮�		
		int color = Color.HSVToColor(hsv);
		int acolor = Color.HSVToColor(0, hsv);
		//閸掓繂顫愰崠鏍拷閺勫骸瀹崇痪鎸庯拷濞撳弶鐓嬮崳锟�	
		...
		//閸掓繂顫愰崠鏍拷閺勫骸瀹抽柅澶嬪閸ｃ劑锟介幏鈺傛蒋娴ｅ秶鐤�		
